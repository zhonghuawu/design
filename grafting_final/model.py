...
def C_grad_std(x, w, X, Y, threshold):
    ...
    eps = 0.01
    #points_number = 20
    #points = random_sample(points_number)*eps-0.5*eps
    points = np.array((-eps, eps))
...
def update_wegiht(w, X, Y, threshold):
    #F = partial(C, X=np.hstack((X, x)), Y=Y, threshold=threshold)
    #result = optimize.fmin_cg(F, np.hstack((w, 1)), disp=0, full_output=1)
    results = optimize.fmin_cg(f=C, x0=w, args=(X, Y, threshold), disp=0, full_output=1)
    wopt = results[0]
    fopt = results[1]
    #func_calls = result[2]
    #grad_calls = result[3]
